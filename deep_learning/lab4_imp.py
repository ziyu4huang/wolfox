...
# %%RUN_CELL <3> train_flow


def train(input_tensor, target_tensor, encoder: EncoderRNN, decoder: DecoderRNN,
...
# %%RUN_CELL <4> start train - command line mode
if _int_mode is not True:
    encoder1 = EncoderRNN(vocab_size, hidden_size).to(device)
    decoder1 = DecoderRNN(hidden_size, vocab_size).to(device)
    trainIters(encoder1, decoder1, n_iters=75000, print_every=5000)


# %%RUN_CELL <5> begin train interactive-run
...
# %%RUN_CELL <6> train batch interactive - encode
if _int_mode is True:
    iter = 1

    def convert_word(word: str, device='cpu'):
        a = [ord(x)-ord('a') + 2 for x in word.lower()] + [1]
        return torch.tensor(a, device=device).view(-1, 1)

    input_tensor = convert_word("consist", device)
    target_tensor = convert_word("consists", device)
    ...
    for ei in range(input_length):
        ...
        v = encoder_outputs[ei] = encoder_output[0, 0]
        # print(v)
...
# %%RUN_CELL <7> train batch interactive - decode
if _int_mode is True:
